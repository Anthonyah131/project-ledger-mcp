...
/**
 * Creates a per-user MCP API client that always forwards the fixed service token.
 */
export function createMcpApiClient(userId: string): McpApiClient {
  return async (path, params) => {
    ...
    const response = await fetch(url.toString(), {
      headers: {
        Authorization: `Bearer ${config.mcpServiceToken}`,
        "X-User-Id": userId,
